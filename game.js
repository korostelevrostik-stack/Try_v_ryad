// ================================================================
//  game.js — ПОЛНАЯ ЛОГИКА ИГРЫ "ТРИ В РЯД"
// ================================================================
...
function getMatches() {
    ...
    for (let i = 0; i < SIZE; i++) {
        for (let j = 0; j < SIZE; j++) {
            ...
            if (right - left + 1 >= 3) {
                for (let c = left; c <= right; c++) { group.push([i, c]);
                    checked.add(i + ',' + c); }
            }
            let top = i,
                bottom = i;
            ...
            if (bottom - top + 1 >= 3) {
                for (let r = top; r <= bottom; r++) {
                    if (!checked.has(r + ',' + j)) { group.push([r, j]);
                        checked.add(r + ',' + j); }
                }
            }
...
function activateBonus(row, col) {
    ...
    if (bonus === BONUS_TYPES.ROCKET) {
        ...
        for (let i = 0; i < SIZE; i++) {
            if (typeof grid[row][i] === 'number') { grid[row][i] = -1;
                score += 2; }
        ...
        for (let j = -2; j <= 2; j++) {
                ...
                if (nr >= 0 && nr < SIZE && nc >= 0 && nc < SIZE) {
                    if (typeof grid[nr][nc] === 'number') { grid[nr][nc] = -1;
                        score += 1; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (restartBtn) {
        restartBtn.addEventListener('click', function() {
            document.getElementById('gameOver').classList.remove('active');
            if (document.getElementById('restartBtn').textContent === '▶ Следующий уровень') {
                startGame();
            } else {
                startGame();
            }
